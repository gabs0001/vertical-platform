class Player extends Sprite {
    ...
    shouldPanCameraToTheLeft({ canvas, camera }) {
        ...
        if(cameraboxRightSide >= scaledDownCanvasWidth + Math.abs(camera.position.x)) camera.position.x -= this.velocity.x
    }

    shouldPanCameraToTheRight({ canvas, camera }) {
        if(this.camerabox.position.x <= 0) return
        if(this.camerabox.position.x <= Math.abs(camera.position.x)) camera.position.x -= this.velocity.x
    }

    shouldPanCameraDown({ canvas, camera }) {
        if(this.camerabox.position.y + this.velocity.y <= 0) return
        if(this.camerabox.position.y <= Math.abs(camera.position.y)) camera.position.y -= this.velocity.y
    }
